...
interface CustomWeatherAPI {
    ...
    suspend fun getAlertByZoneIdentifier(@Path("zoneId") zoneIdentifier: String): Response<WeatherAlertsResponse>

    @GET("/alerts/active/region/{region}")
    suspend fun getAlertByRegion(@Path("region") region: String): Response<WeatherAlertsResponse>

    @GET("/gridpoints/{weatherForecastOfficeID}/{latitudeCoordinate}/{longitudeCoordinate}/forecast")
    ...
    @GET("/gridpoints/{weatherForecastOfficeID}/{latitudeCoordinate}/{longitudeCoordinate}/forecast/hourly")
